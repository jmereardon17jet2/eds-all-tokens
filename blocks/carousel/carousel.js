...
function updateActiveSlide(slide) {
  ...
  block.querySelector('.carousel-navigation-buttons .carousel-slide-number').textContent =
    slideIndex + 1;
}
...
function createSlide(row, slideIndex, carouselId) {
  ...
  row.querySelectorAll(':scope > div').forEach((col) => {
    ...
    if (icon) {
      const elements = [...col.children].filter(
        (el) => !el.firstChild?.classList?.contains('icon')
      );
